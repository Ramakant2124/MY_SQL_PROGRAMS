...
def insert(rn, name):
    cursor.execute(f'insert into stud2 values({rn},"{name}")')
    print("Data Inserted Succesfully")
def update(rn,name):
    cursor.execute(f'update stud2 set name="{name}" where id={rn};')
    print("Data Updated Succesfully")

def delete(rn):
    cursor.execute(f'delete from stud2 where id={rn};')
    print("Data Delete Succesfully")

def show():
    cursor.execute('select * from stud2;')
    for row in cursor:
         print(row)
try:         
    ...
    cursor.execute("CREATE TABLE stud2(id INT, name VARCHAR(32));")
